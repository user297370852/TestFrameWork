...
只调用 Test_oracles.BASE_ORACLES，生成面向排查人员阅读的简洁报告。
"""
...
try:
    from Test_oracles import BASE_ORACLES
except ImportError:
...
class BasicAnalyzer:
    def __init__(self, oracles: Optional[List[Any]] = None):
        self.oracles = oracles if oracles is not None else BASE_ORACLES
    ...
    def _extract_info(self, anomaly: Dict[str, Any]) -> List[str]:
        ...
        for item in self._iter_leaf_anomalies(anomaly):
            ...
            elif self._looks_like_triggered_anomaly(item):
                infos.append(self._fallback_info(anomaly, item))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="基础测试预言极简报告生成器")
    parser.add_argument("input_dir", help="包含JSON测试记录的目录")
    parser.add_argument("-o", "--output", default="basic_report.json", help="输出报告路径")
    parser.add_argument("--oracle", help="只运行指定基础测试预言函数，例如 oracle_test_failure")
    ...
    if not BASE_ORACLES:
        print("错误: 没有可用的基础测试预言，请检查 Test_oracles 模块")
        return

    selected_oracles = BASE_ORACLES
    if args.oracle:
        selected_oracles = [oracle for oracle in BASE_ORACLES if oracle.__name__ == args.oracle]
        if not selected_oracles:
            available = ", ".join(oracle.__name__ for oracle in BASE_ORACLES)
            print(f"错误: 未找到基础测试预言 '{args.oracle}'，可用预言: {available}")
            return

    analyzer = BasicAnalyzer(selected_oracles)
    ...
    print(f"基础预言分析完成，共发现 {len(anomalies)} 个触发结果")
